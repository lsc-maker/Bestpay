...
def gen_user_amount_features(df):
    ...
    df['user_amount_std'] = df.groupby(['user'])['amount'].transform('std')
    df['user_amount_max'] = df.groupby(['user'])['amount'].transform('max')
    df['user_amount_min'] = df.groupby(['user'])['amount'].transform('min')
    df['user_amount_sum'] = df.groupby(['user'])['amount'].transform('sum')
    df['user_amount_med'] = df.groupby(['user'])['amount'].transform('median')
    df['user_amount_cnt'] = df.groupby(['user'])['amount'].transform('count')
    return df
...
def gen_user_nunique_features(df, value, prefix):
    ...
    # }).reset_index()
    df['user_{}_{}_nuniq'.format(prefix, value)] = df.groupby(['user'])[value].transform('nunique')
    return df
